...
transformed parameters {
  ...
  real<lower=0> tau = sigma_s;                     // Horseshoe hyperparameters
  vector<lower=0>[7] lambda_tilde=sqrt(caux*square(tau*lambda)./(caux +square(tau *lambda)));                 // ’ truncated ’ local shrinkage parameter
  vector[7] beta_  = beta_raw .* lambda_tilde;
  real beta_x = beta_[1];
  vector[3] beta_y= beta_[2:4];
  vector[3] beta_xy= beta_[5:7];
...
model {
  ...
  beta_0 ~ normal(0,10); 
  beta_raw ~ std_normal();                       // Horseshoe hyperprior
  lambda ~ cauchy(0,1);
