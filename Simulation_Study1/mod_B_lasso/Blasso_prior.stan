...
parameters {
  ...
  real<lower=0> beta_0;
  vector[P] beta_eta; //vector[P] beta_raw;                              // B-Lasso hyperparameters
  real<lower=0> lambda;
...
transformed parameters {
  ...
  // vector[N] u= u_raw*sigma_u;
  // real<lower=0> laplace_sc = sigma_u / lambda;    // B-Lasso hyperparameters
  // vector[P] beta_std = sigma_u * beta_raw;
  //vector[P] beta_eta = beta_raw * (sigma_u/lambda);
