...
parameters {
  ...
  real<lower=0> beta_0;
  real<lower=0> aux_1;                     // Horseshoe hyperparameters
  real<lower=0> aux_2;
...
transformed parameters {
  ...
  real<lower=0> tau = aux_1 * sqrt(aux_2) * sigma_s; 
  vector<lower=0>[27] lambda_tilde=sqrt(caux*square(lambda)./(caux +square(tau *lambda)));     // ’ truncated ’ local shrinkage parameter
  vector[27] beta_  = beta_raw .* lambda_tilde * tau;
  real beta_x = beta_[1];
...
model {
  ...
  beta_0 ~ normal(0,10); 
  lambda ~ cauchy(0,1);     // Horseshoe hyperprior
  beta_raw ~ std_normal();             
  caux ~ inv_gamma(2,8);
  aux_1 ~ std_normal();             
  aux_2 ~ inv_gamma(1.5,1.5);
