...
transformed parameters {
  ...
  vector<lower=0>[27] lambda_tilde=sqrt(caux*square(tau*lambda)./(caux +square(tau *lambda)));                 // ’ truncated ’ local shrinkage parameter
  vector[27] beta_  = beta_raw .* lambda_tilde;
  real beta_x = beta_[1];
