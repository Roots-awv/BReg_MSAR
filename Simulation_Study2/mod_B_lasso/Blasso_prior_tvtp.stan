...
parameters {
  ...
  real<lower=0> beta_0;
  vector[N] eps_x;
  vector[P] beta_x;                              // B-Lasso hyperparameters
  vector[P] beta_xz; 
  real beta_z; 
  real<lower=0> lambda_x;                        
  real<lower=0> lambda_xz;
...
transformed parameters {
  ...
  //real<lower=0> laplace_scx = sigma_x/lambda_x;   // B-Lasso hyperparameters
  //real<lower=0> laplace_scxz = sigma_x/lambda_xz;  
  //real<lower=0> laplace_scz = sigma_x/lambda_z;  
...
model {
  ...
  // Between-level
  beta_0 ~ cauchy(0,5); 
  lambda_x ~ cauchy(0,2.5);                      // B-Lasso hyperparameters
  beta_x~ double_exponential(0,sigma_x/lambda_x);
